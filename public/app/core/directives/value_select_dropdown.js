  ...
  function (angular, _, coreModule) {
    ...
    coreModule.default.controller('ValueSelectDropdownCtrl', function ($q) {
      ...
      vm.queryChanged = function () {
        ...
        if (variable != null && variable.type === "query" && variable.realtime && vm.search.query.length >= variable.realtimeMinInput) {
          var inputQuery = vm.search.query + "*";
          var oldQuery = variable.query;
          var json = JSON.parse(oldQuery);
          if (json.field != null) {
            json.query = json.field + ":" + inputQuery + " AND " + json.query;
            json.size = variable.realtiemSize;
            var realtimeQuery = JSON.stringify(json);
            //alert(jsonStr);
            variable.datasourceSrv.get(variable.datasource).then(function (ds) {
              ds.metricFindQuery(realtimeQuery).then(function (results) {
                var options = variable.metricNamesToVariableValues(results);
                if (!options.length) {
                  options.push({text: 'None', value: '', isNone: true});
                }
                vm.options = options;
                vm.search.options = options;
